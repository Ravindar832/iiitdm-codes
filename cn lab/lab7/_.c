...
int split_ip(const char ip[], char ipArray[4][4])
{
    char temp[4];
    int count = 0;
    ...
    strcpy(ipArray[3], temp);
    return count + 1;
}

int ip_validate(char ipArray[4][4], int count)
{
    if (count > 4)
    {
        return 1;
    ...
    for (int i = 0; i < 4; i++)
    {
        if (atoi(ipArray[i]) < 0 || atoi(ipArray[i]) > 255)
        {
...
void ip_binary(char ipArray[4][4])
{
    for (int i = 0; i < 4; i++)
    {
        if (i == 1 || i == 2 || i == 3)
...
int main()
{
    ...
    int count = split_ip(ip, ipArray);

    int flag = ip_validate(ipArray, count);
    if (flag == 0)
        printf("Valid IP!!\n");
    else
    {
    ...
    int class = ip_class(ipArray[0]);

    if (class < 4)
    {
        int sn;
        printf("\nEnter the number of subnets {should be in power of two}: ");
        scanf("%d", &sn);
    ...
    {
        printf("This network cannot be divided into subnets.\n");
        exit(0);
    }
